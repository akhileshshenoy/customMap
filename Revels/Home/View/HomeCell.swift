...
class HomeCell: UICollectionViewCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    @objc var eventsFilePath: String {
        let manager = FileManager.default
        let url = manager.urls(for: .documentDirectory, in: .userDomainMask).first
        return url!.appendingPathComponent("Events").path
    }
    
    private func saveData(events: [Event], schedule: [Schedule]){
        NSKeyedArchiver.archiveRootObject(events, toFile: eventsFilePath)
        NSKeyedArchiver.archiveRootObject(schedule, toFile: scheduleFilePath)
    }
...
//        backgroundColor = .black
        ...
        _ = title.anchor(topAnchor, left: leftAnchor, bottom: nil, right: rightAnchor, topConstant: 16, leftConstant: 16, bottomConstant: 16, rightConstant: 16, widthConstant: 0, heightConstant: 24)
...
//            cell.roundLabel.text = "Round \(dayArray[indexPath.item].eventRound!)"
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: self.frame.width-72, height: 150)
    }
...
extension HomeCell {
    ...
    private func getData(){
        if let retrievedEventsData = NSKeyedUnarchiver.unarchiveObject(withFile: eventsFilePath) as? [Event]{
            ...
            if let retrievedSchedulesData = NSKeyedUnarchiver.unarchiveObject(withFile: scheduleFilePath) as? [Schedule]{
                self.schedule = retrievedSchedulesData
                self.makeSchedule()
            }else{
                self.getSchedule()
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                self.getEvents()
                print("Refetching events and schedule and caching new data")
            }
        }else{
                ...
                if let data = response.data{
                    do{
                        ...
                        if let schedule = response.data{
                            ...
                            self.saveData(events: self.events, schedule: self.schedule)
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0) {
