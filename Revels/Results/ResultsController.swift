...
class ResultsController: UITableViewController {
    ...
    var sectionDictionary = [String: [SectionState]]()
    var sectionSectionTitles = [String]()
    ...
    func setupDictionary() {
        self.sectionArray.sort{ $0.name < $1.name}
        
        for section in sectionArray {
            let sectionKey = String(section.name.prefix(1))
            if var sectionValues = sectionDictionary[sectionKey] {
                sectionValues.append(section)
                sectionDictionary[sectionKey] = sectionValues
            } else {
                sectionDictionary[sectionKey] = [section]
            }
        }
        
        sectionSectionTitles = [String](sectionDictionary.keys)
        sectionSectionTitles.sort{ $0 < $1 }
    }
...
//            button.setImage(UIImage(named: "uparrow"), for: .normal)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.item == 0 {
            ...
            var unsortedResultsArray = sectionArray[indexPath.section].results
            unsortedResultsArray.sort { $0.position < $1.position}
            let result = unsortedResultsArray[indexPath.item-1]
