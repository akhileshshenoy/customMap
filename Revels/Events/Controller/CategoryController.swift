...
class CategoryController: UITableViewController {
    ...
    func getEvents() {
        ...
        URLSession.shared.dataTask(with: url) { (data, response, err) in
            ...
            do {
                // success
                self.eventArray = try JSONDecoder().decode(EventResponse.self, from: data).data!
    ...
    @objc func callButtonTapped(_ sender: UIButton!) {
        ...
        let actionSheet = UIAlertController(title: data[indexPath].name, message: "Call Category Heads", preferredStyle: .actionSheet)
        
        actionSheet.addAction(UIAlertAction(title: data[indexPath].cc1_name, style: UIAlertAction.Style.default, handler: { (action) -> Void in
            guard let number = URL(string: "tel://" + self.data[indexPath].cc1_contact!) else { return }
            UIApplication.shared.open(number)
        }))
        
        actionSheet.addAction(UIAlertAction(title: data[indexPath].cc2_name, style: UIAlertAction.Style.default, handler: { (action) -> Void in
